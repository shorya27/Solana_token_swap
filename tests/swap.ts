...
describe("swap", async () => {
  ...
  it("Puts the tokens Alice offers into the vault when Alice makes an offer", async () => {
    ...
    const offerId = getRandomBigNumber();
     console.log("TEST → Passing token_program:", TOKEN_2022_PROGRAM_ID.toBase58());
  console.log("TEST → Passing tokenMintA   :", accounts.tokenMintA.toBase58());
    ...
    const offer = PublicKey.findProgramAddressSync(
      [
        ...
        Buffer.from(offerId.toArray("le", 8))  // ensures it's exactly 8 bytes
      ],
      program.programId
    )[0];
